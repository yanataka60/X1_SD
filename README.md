...
| ------------ | ------------ |
|BASIC CZ-8CB01||
|S-OS SWORD TAPE版|Oh!MZ '87/3月号|
|EDAS       |月刊I/O '84/6月号|
|TTLインタプリタ|Oh!MZ '84/10月号|
|X1エディタ・アセンブラ|マシン語プログラム入門|
...
## 操作方法
　電源ON、又はリセットでX1_SD BOOT LOADERのコマンド入力待ちになるので以下のコマンドが使えます。

　X1のMONITORで使えるD,M,G,#コマンドは同じように使えます。
...
　起動直後に必ずCLEAR &HF800又はLIMIT &HF800を実行してください。SDカードアクセスルーチンがF800Hから配置されています。
...
### S-OS SWORD
　Oh!MZ '87/3月号に掲載されたS-OS SWORDをX1_SD対応とすることでOh!MZ誌及びOh!X誌に連載されたTHE SENTINELのプログラムがFDD、CMT無しでも利用できるようになります。
